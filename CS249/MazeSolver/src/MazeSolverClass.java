public class MazeSolverClass
   {
    ...
    /**
     * Public method for solving maze. prints out a success message if it found
     * a solution and prints out a failure message if it doesn't find a solution
     * <p>
    ...
    public boolean solveMaze()
       {
           LocationData firstLoc = new LocationData(getInitialDirection(), startY, startX);
           LocationData startingLoc = setFirstPosition(firstLoc, TRACK_MARKER);

           boolean returnValue = solveMazeHelper(startingLoc);
           ...
           else
           {
               ...
               undoMoveToCell(firstLoc);
               undoMoveToCell(startingLoc);
           }
     ...
     * maze mover location status. It's a left hand algorithim so it checks left
     * then forward then right.
     * 
     * @param currentLoc LocationData holding maze mover location status
     * 
     * @return Boolean result of attempt to find path through maze. Will return
     * a true if there is a solution and false if it fails to find a solution.
     */
    public boolean solveMazeHelper(LocationData currentLoc)
    {
        LocationData workingLoc = currentLoc;
        boolean returnValue = false;

        //checks left
        workingLoc = turnToLeft(workingLoc);
        if (isExitAhead(workingLoc))
        {
            moveToExit(workingLoc);
            diagnosticDump();
            return true;
        }

        if (!isWallAhead(workingLoc))
        {
            workingLoc = moveToNextCell(workingLoc);
            diagnosticDump();
            returnValue = solveMazeHelper(workingLoc);

            //resets the location of the copy to its starting coordinates
            if (!returnValue)
            {
                workingLoc.xPos = currentLoc.xPos;
                workingLoc.yPos = currentLoc.yPos;
                workingLoc = undoMoveToCell(workingLoc);
                diagnosticDump();
                workingLoc.xPos = currentLoc.xPos;
                workingLoc.yPos = currentLoc.yPos;
            }
        ...
        //checks forward
        workingLoc = turnToRight(workingLoc);
        if (isExitAhead(workingLoc))
        {
            moveToExit(workingLoc);
            diagnosticDump();
        ...
        if (!isWallAhead(workingLoc) && !returnValue)
        {
            workingLoc = moveToNextCell(workingLoc);
            diagnosticDump();
            returnValue = solveMazeHelper(workingLoc);
